...
class ResidentialAvmV2Client
{
    public function predict(Property $property): array
    {
        $baseUrl = rtrim((string) config('services.avm_v2.url'), '/');

        if ($baseUrl === '') {
            throw new AvmClientException('avm_v2_not_configured', 'El microservicio AVM v2 no está configurado.');
        }

        $payload = $this->payload($property);

        try {
            $request = Http::timeout((int) config('services.avm_v2.timeout', 20))
                ->acceptJson()
                ->asJson();

            if ($token = config('services.avm_v2.token')) {
                $request = $request->withToken($token);
            }

            $response = $request->post($baseUrl.'/predict/v2/residential', $payload);
        } catch (ConnectionException $exception) {
            Log::warning('AVM v2 service connection failed', ['message' => $exception->getMessage()]);
            throw new AvmClientException('avm_v2_connection_failed', 'No fue posible conectar con AVM v2.', $exception);
        }

        if ($response->failed()) {
            Log::warning('AVM v2 service returned an error', [
                'status' => $response->status(),
                'body' => $response->body(),
            ]);

            throw new AvmClientException('avm_v2_http_error', 'AVM v2 devolvió un error HTTP '.$response->status().'.');
        }

        try {
            $data = $response->json() ?? json_decode($response->body(), true, flags: JSON_THROW_ON_ERROR);
        } catch (JsonException $exception) {
            throw new AvmClientException('avm_v2_invalid_json', 'AVM v2 devolvió JSON inválido.', $exception);
        }

        if (! is_array($data) || ! array_key_exists('eligible', $data)) {
            throw new AvmClientException('avm_v2_incomplete_response', 'AVM v2 no devolvió una respuesta válida.');
        }
