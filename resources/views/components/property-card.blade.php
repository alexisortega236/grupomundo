...
<article class="group overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-[#e4dccd] transition hover:-translate-y-1 hover:shadow-xl">
    <div class="relative">
        <img class="h-56 w-full object-cover" src="{{ $property->coverUrl() }}" alt="{{ $property->title }}">
        ...
        <span class="absolute bottom-4 left-4 rounded-full bg-white px-4 py-2 font-semibold text-[#0d2723] shadow">{{ $property->formattedPrice() }} @if($property->operation_type->value === 'rent')<small>/ {{ $property->rent_period ?: 'mes' }}</small>@endif</span>
    </div>
    <div class="p-5">
        ...
        <div class="mt-4 flex flex-wrap gap-3 text-sm text-[#687773]">
            <span>{{ $property->bedrooms ?? 0 }} rec.</span><span>{{ $property->bathrooms ?? 0 }} banos</span><span>{{ $property->construction_area ? number_format($property->construction_area).' m2' : 'Sup. por confirmar' }}</span>
        </div>
