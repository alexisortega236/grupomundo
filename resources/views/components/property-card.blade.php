...
<article class="group overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
    <div class="relative aspect-[4/3] overflow-hidden bg-slate-100">
        ...
        <span class="absolute left-4 top-4 rounded-full bg-[#082142] px-4 py-2 text-xs font-extrabold text-white">{{ $property->operation_type->label() }}</span>
        @if($property->is_featured)<span class="absolute right-4 top-4 rounded-full bg-[#d3aa56] px-3 py-2 text-xs font-bold text-[#082142]">Destacada</span>@endif
        <span class="absolute bottom-4 left-4 rounded-xl bg-white px-4 py-2 font-extrabold text-[#082142] shadow">{{ '$'.number_format($property->price, 0) }} {{ $property->currency }}{{ $property->operation_type->value === 'rent' ? ' / '.($property->rent_period ?: 'mes') : '' }}</span>
    </div>
    <div class="p-6">
