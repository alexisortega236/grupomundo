...
<header x-data="{ open: false }" class="sticky top-0 z-40 border-b border-[#e6dfd2] bg-white/95 backdrop-blur">
    <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-5 sm:px-6 lg:px-8">
        <a href="{{ route('home') }}" class="flex items-center gap-3.5">
            <span class="grid h-12 w-12 place-items-center border border-[#c9a968] font-serif text-xl text-[#c9a968]">M</span>
        ...
        <nav class="hidden items-center gap-8 text-sm font-medium text-[#51635f] md:flex">
            ...
            <a class="rounded-full bg-[#d5b673] px-5 py-3 text-xs font-bold uppercase tracking-[.16em] text-[#0d2723] transition hover:bg-[#c9a968]" href="{{ $wa }}" target="_blank" rel="noopener">Hablar con un asesor</a>
        </nav>
    </div>
    <nav x-show="open" x-cloak class="grid gap-2 border-t border-[#e6dfd2] bg-white px-4 pb-4 text-sm md:hidden">
        ...
        <a class="rounded-full bg-[#d5b673] px-5 py-3 text-center text-xs font-bold uppercase tracking-[.16em] text-[#0d2723]" href="{{ $wa }}" target="_blank" rel="noopener">Hablar con un asesor</a>
    </nav>
</header>
