    ...
    <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div class="grid gap-8 lg:grid-cols-[1.1fr_.9fr]">
            ...
            <aside class="rounded-lg bg-white p-7 shadow-sm">
                ...
                @endif
                <dl class="mt-6 grid grid-cols-2 gap-4 text-sm"><div><dt>Recámaras</dt><dd class="font-bold">{{ $property->bedrooms ?? '-' }}</dd></div><div><dt>Baños</dt><dd class="font-bold">{{ $property->bathrooms ?? '-' }}</dd></div><div><dt>Estacionamientos</dt><dd class="font-bold">{{ $property->parking_spaces ?? '-' }}</dd></div><div><dt>Construcción</dt><dd class="font-bold">{{ $property->construction_area ?? '-' }} m²</dd></div><div><dt>Terreno</dt><dd class="font-bold">{{ $property->land_area ?? '-' }} m²</dd></div><div><dt>Antigüedad</dt><dd class="font-bold">{{ $property->age ?? '-' }}</dd></div></dl>
                <a class="mt-8 block rounded-full bg-[#0d2723] px-6 py-3 text-center font-bold text-white" href="{{ $wa }}" target="_blank" rel="noopener">Solicitar información por WhatsApp</a>
            </aside>
        </div>
        <div class="mt-12 grid gap-8 lg:grid-cols-[1fr_380px]">
            ...
            <form method="POST" action="{{ route('contact-requests.store') }}" class="rounded-lg bg-white p-7 shadow-sm">@csrf<input type="hidden" name="property_id" value="{{ $property->id }}"><input class="hidden" name="website" tabindex="-1" autocomplete="off"><h2 class="font-serif text-3xl">Solicitar información</h2><div class="mt-5 grid gap-4"><x-form.input label="Nombre" name="name" /><x-form.input label="Teléfono" name="phone" /><x-form.input label="Correo electrónico" name="email" type="email" /><x-form.textarea label="Mensaje" name="message" :value="'Hola, me interesa la propiedad '.$property->title.'.'" /><button class="rounded bg-[#d5b673] px-5 py-3 font-bold text-[#0d2723]">Enviar solicitud</button></div></form>
        </div>
