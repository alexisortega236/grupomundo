...
@php($coverImage = $property->coverImage->first() ?: $property->images->first())
    <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div class="mt-12 grid gap-8 lg:grid-cols-[1fr_380px]">
            ...
            <form method="POST" action="{{ route('contact-requests.store') }}" class="rounded-lg bg-white p-7 shadow-sm">@csrf<input type="hidden" name="property_id" value="{{ $property->id }}"><input class="hidden" name="website" tabindex="-1" autocomplete="off"><p class="text-xs font-bold uppercase tracking-[.2em] text-[#b89752]">Otra forma de contacto</p><h2 class="mt-2 font-serif text-3xl">Solicitar información</h2><div class="mt-5 grid gap-4"><x-form.input label="Nombre" name="name" /><x-form.input label="Teléfono" name="phone" /><x-form.input label="Correo electrónico" name="email" type="email" /><x-form.textarea label="Mensaje" name="message" :value="'Hola, me interesa la propiedad '.$property->title.'.'" /><button class="rounded bg-[#d5b673] px-5 py-3 font-bold text-[#0d2723]">Enviar solicitud</button></div></form>
        </div>
